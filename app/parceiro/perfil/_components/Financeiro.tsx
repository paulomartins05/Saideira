...
import FiltroEExportacao from "./FiltroEExportacao";

export default async function Financeiro({ usuarioId, periodo = "todos" }: { usuarioId: string, periodo?: string }) {
    ...
    if (dataLimite) {
        whereClause.updatedAt = { gte: dataLimite };
    }
    ...
    const resgatesSimples = resgatesConcluidos.map(r => ({
        id: r.id,
        titulo: r.oferta.titulo,
        precoResgate: Number(r.oferta.precoResgate),
        updatedAtStr: r.updatedAt.toISOString(),
    }));
    ...
    return (
        <div className="bg-card p-6 md:p-8 rounded-[20px] border border-line flex flex-col">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-10">
                <div className="bg-amber/10 p-6 rounded-[16px] border border-amber/30 flex flex-col justify-center">
                    <p className="text-[12px] uppercase tracking-widest font-bold text-amber-dark mb-1">Total no Período</p>
                    <p className="font-display font-extrabold text-[32px] text-night">
                        R$ {saldo.toFixed(2).replace('.', ',')}
                    </p>
                </div>

                <div className="bg-paper p-5 rounded-[16px] border border-line flex flex-col justify-center items-start">
                    <FiltroEExportacao resgates={resgatesSimples} />
                </div>
            </div>
            ...
            <div className="flex flex-col gap-3">
                {resgatesConcluidos.length === 0 ? (
                    <p className="text-[13.5px] text-center text-muted py-8">Nenhuma transação encontrada nesse período.</p>
                ) : (
                    resgatesConcluidos
                        .map(resgate => (
