...
import { criarOferta, editarOferta } from "@/app/actions/ofertas";
import { authClient } from "@/lib/auth-client";
import { appToast } from "@/lib/toast";
import { Utensils, Store, Tag, Scale, MapPin } from "lucide-react";
...
export default function FormOferta({ ofertaInicial }: FormOfertaProps) {
    const router = useRouter();
    const { data: session } = authClient.useSession();

    const tipoNegocioUsuario = (session?.user as any)?.tipoNegocio || "OUTRO";
    ...
    const [imagemPreviews, setImagemPreviews] = useState<string[]>(ofertaInicial?.imagemUrl || []);
    ...
    const {
        ...
        defaultValues: {
            nome: ofertaInicial?.titulo || "",
            descricao: ofertaInicial?.descricao || "",
            categoria: ofertaInicial?.categoria || "",
            precoOriginal: ofertaInicial ? formatCoinInput(ofertaInicial.precoOriginal.toFixed(2).replace(".", "")) : "",
            precoResgate: ofertaInicial ? formatCoinInput(ofertaInicial.precoResgate.toFixed(2).replace(".", "")) : "",
            quantidade: ofertaInicial?.quantidade || 1,
            peso: ofertaInicial?.peso ? ofertaInicial.peso.toString().replace(".", ",") : "",
            validade: ofertaInicial ? formatarDataLocal(ofertaInicial.dataValidade) : "",
            termosAceitos: ofertaInicial ? true : false,
        },
    });
    ...
    const onSubmit = async (data: NovoResgateFormInputs) => {
        if (!ofertaInicial && imagemFiles.length === 0) {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="bg-card border border-line rounded-[24px] p-6 md:p-10 max-w-3xl mx-auto flex flex-col gap-10 shadow-sm">
            ...
            <div className="bg-night rounded-2xl p-4 flex items-center gap-4">
                ...
                <div className="flex flex-col">
                    <span className="text-[12px] text-muted font-medium tracking-widest uppercase">
                        {ofertaInicial ? "Editando como" : "Publicando como"}
                    </span>
            ...
            <div className="flex flex-col gap-6">
                ...
                <FormGroup label={ofertaInicial ? "Fotos (Envie novas para substituir as antigas)" : "Fotos (Máx. 3)"}>
            ...
            <div className="flex flex-col gap-5 pt-4">
                ...
                <button type="submit" disabled={isSubmitting} className={`w-full bg-amber hover:bg-amber-dark text-night font-bold text-[15px] py-4 rounded-xl transition-all ${isSubmitting ? "opacity-50 cursor-not-allowed" : ""}`}>
                    {isSubmitting ? "SALVANDO..." : (ofertaInicial ? "Salvar Alterações" : "Publicar Oferta")}
                </button>
            </div>
        </form>
    );
